...
#+begin_src emacs-lisp :var gvpr=call-graph-gvpr :display=none :lexical t :results none
  (defun call-graph-ii ()
    (interactive)
    (let ((byte-compile-generate-call-tree t)
	  (base (file-name-base (buffer-file-name))))
...
#+begin_example gvpr
BEG_G {label="Call graph for " + prefix + ". Green are commands, : stands for package prefix."}
N{ shape="box"}
N [command=="t"] {style="filled, rounded", fillcolor="green"}
N [0==match(name, $O.prefix)]{URL="help:" + name ,label=":"+substr(name, (1 + length($O.prefix)))}
N [0!=match(name, $O.prefix)] {delete(root, $)}
#+end_example
